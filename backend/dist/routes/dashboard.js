...
router.get('/', async (req, res) => {
    try {
        ...
        const totals = await (0, init_1.dbGet)(`
      SELECT 
    ...
    `);
        ...
        const categorySpending = await (0, init_1.dbAll)(`
      SELECT 
        c.name,
        c.color,
        c.icon,
    ...
    `);
        ...
        const recentTransactions = await (0, init_1.dbAll)(`
    ...
    `);
        ...
        const dailyTrend = await (0, init_1.dbAll)(`
      SELECT 
        DATE(t.date) as date,
    ...
    `);
        ...
        const currentMonth = await (0, init_1.dbGet)(`
      SELECT 
        COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as income,
        COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as expenses
      FROM transactions t
    ...
    `);
        const previousMonth = await (0, init_1.dbGet)(`
      SELECT 
        COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as income,
        COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as expenses
      FROM transactions t
    ...
    `);
        ...
        const goalsSummary = await (0, init_1.dbAll)(`
      SELECT 
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        const weeklyPattern = await (0, init_1.dbAll)(`
      SELECT 
    ...
    `, params);
        ...
        const monthlyTrend = await (0, init_1.dbAll)(`
      SELECT 
    ...
    `, params);
        // Top spending days
        const topSpendingDays = await (0, init_1.dbAll)(`
      SELECT 
        DATE(t.date) as date,
