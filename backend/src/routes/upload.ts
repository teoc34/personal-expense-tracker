    ...
    for (let i = 0; i < transactions.length; i++) {
      try {
        ...
        if (auto_categorize === 'true') {
          categoryId = await categorizeTransaction(transaction.description, transaction.amount, transaction.type);
        }

        // Insert transaction into database
        const result = await dbRun(
          'INSERT INTO transactions (amount, description, category_id, type, date) VALUES (?, ?, ?, ?, ?)',
          [transaction.amount, transaction.description, categoryId, transaction.type, transaction.date]
        );

        // Get the created transaction with category info
        const createdTransaction = await dbGet(`
...
router.post('/validate', upload.single('file'), async (req, res) => {
  try {
    ...
    const transactions = await parseCSVFile(filePath, true); // Pass true for preview mode
