...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await dbRun(
      'INSERT INTO goals (title, target_amount, current_amount, category_id, target_date, status) VALUES (?, ?, ?, ?, ?, ?)',
      [title.trim(), target_amount, current_amount, category_id || null, target_date || null, status]
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existingGoal = await dbGet('SELECT * FROM goals WHERE id = ?', [id]);
    if (!existingGoal) {
      return res.status(404).json({ error: 'Goal not found' });
    }
