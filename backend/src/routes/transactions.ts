...
router.get('/', async (req, res) => {
  try {
    ...
    const { total } = await dbGet(countQuery, countParams) as { total: number };
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await dbRun(
      ...
      [amount, description, category_id || null, type, date]
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existingTransaction = await dbGet('SELECT * FROM transactions WHERE id = ?', [id]);
    ...
    await dbRun(
      ...
      [amount, description, category_id || null, type, date, id]
    );
